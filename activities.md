...
> 最終更新: 2026-08-17 19:18 JST  
> 収集期間: 全履歴（2026-08-17 更新）
...
### 画像バトラー — 「ちょっと作りたかったもの」を生成AIでサッと実現（2026-07）
PhotoBattler（⭐1）の後継として、作りたいアイデアを生成 AI に委ねてあっさり実現した体験を記事化。「プロトタイプ製作のコスト感が変わった」という感覚を具体的なプロセスと共に記録。生成 AI を使いこなす実践者として、制作者の意図と AI の能力の棲み分けを語っている。

### AWS DevOps Agent 雑感（2026-07）
AWS が提供する DevOps Agent の実際の使い心地を検証・記録。GitHub Copilot Coding Agent・Cursor など複数の AI 開発エージェントを実務や個人開発で試し続けてきた蓄積を背景に、AWS エコシステム内エージェントの特性を比較考察した。

### agent-lab-dotnet — GitHub Copilot エージェントラボ参加（2026-04）
Microsoft 公式の GitHub Copilot Agent Lab（Blazor / .NET 10 製ソーシャルビンゴゲーム "Soc Ops"）を自リポジトリに導入。コンテキストエンジニアリング・カスタムエージェント構築・マルチエージェント TDD の 4 ステップラボを実践。Codespaces 環境で手軽に体験できる構成を確認した。

### SRC# に GitHub Agentic Workflows を導入（2026-06）
長期プロジェクト SRC# に自作の `gh-aw-compile-action` を適用し、Agentic Workflows の `.lock.yml` 自動生成を CI に組み込んだ（PR #1032 "Aw compile"）。自ら作ったツールを自らのプロジェクトに即導入するアジャイルな自己適用を実践した。

### 大手外食業の中央注文管理 Web サービスダウン事例考察（2026-05）
大手外食チェーンの注文管理システム障害をエンジニアの観点で分析した記事を公開。アーキテクチャの脆弱性・可用性設計の要点を実例から読み解き、現場エンジニアとしての視座を示した。
...
- [画像バトラー〜ちょっと作りたかったものを生成AIにサッと作って貰う〜](https://koudenpa.hatenablog.com/entry/2026/07/30/223317)（2026-07-30）— 「作りたい」と思ったものを生成 AI に委ねてプロトタイプを実現した体験記
- [AWS DevOps Agent雑感](https://koudenpa.hatenablog.com/entry/2026/07/24/120000)（2026-07-24）— AWS DevOps Agent を実際に試した使い心地の記録
...
- [大手外食業の中央注文管理Webサービスがダウンしたら？](https://koudenpa.hatenablog.com/entry/2026/05/06/153121)（2026-05-06）— 外食チェーンの注文管理システム障害をアーキテクチャ視点で考察した記事
...
| 時期 | マイルストーン |
|------|-------------|
| 2026-07 | はてなブログに AWS DevOps Agent 雑感・画像バトラー記事を公開 |
| 2026-06 | SRC# に gh-aw-compile-action を導入（Agentic Workflows CI 化） |
