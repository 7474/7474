...
### agent-lab-dotnet — GitHub Copilot Agent Lab を Blazor + .NET 10 で体験（2026-04）
GitHub Copilot の Agent Mode・カスタムエージェント・マルチエージェント開発を体験するハンズオンラボに参加。Blazor WebAssembly + .NET 10 で構築したソーシャルビンゴゲーム「Soc Ops」を題材に、コンテキストエンジニアリング・デザインファーストフロントエンド・TDD Red→Green→Refactor をエージェントで自動化する手法を実践した。

### phpMyAdmin on Lambda — Lambda Web Adapter で DB 管理ツールをサーバーレス化（2026-04）
phpMyAdmin を AWS Lambda Web Adapter を使って Lambda Function URL で動かすことに成功。CloudFront (CDN) → Lambda Function URL → Lambda (phpMyAdmin + Apache) という構成で、VPC 内 RDS/Aurora への DB 管理アクセスをサーバーレスで実現。Dockerfile 内でファイルシステムの書き込み先を `/tmp` に集約するなど Lambda の制約に対応する工夫が詰まっている。

### Azure嫌になっちまったな —— 趣味の拠点を AWS CloudFront へ移行（2026-04）
Azure CDN (Classic) がマネージド証明書サポートを 2026-04-14 に打ち切り、後継の Azure Front Door は月額 $35〜 と趣味用途に割高なため AWS CloudFront（完全従量課金・ACM 証明書無料）へ移行。「エンタープライズ全振りで日曜プログラマーの遊び場を失いつつある Azure」への正直な所感を記事にした。
...
### php-my-admin-lambda-web-adapter — phpMyAdmin を AWS Lambda でサーバーレス DB 管理ツールに
- **概要**: Lambda Web Adapter を使って phpMyAdmin を AWS Lambda Function URL で動かす実験プロジェクト。VPC 内の RDS/Aurora に接続する DB 管理ツールを、常時稼働のサーバーを立てずにオンデマンドで起動できる構成を実現した。
- **技術スタック**: PHP, Docker, AWS Lambda, Lambda Web Adapter, CloudFront, Terraform
- **主な成果**:
  - CloudFront → Lambda Function URL → Lambda (phpMyAdmin + Apache) 構成で動作実証
  - Lambda の書き込み制限（読み取り専用ファイルシステム）に対応するため、PID ファイル・ロック・セッション・TempDir をすべて `/tmp` 配下に集約
  - amd64/arm64 のマルチアーキテクチャ Docker イメージをビルド・プッシュ
  - Terraform サンプルで Lambda のタイムアウト・メモリ最適化（30秒、512 MB）を提示
...
- [Azure嫌になっちまったな —— 趣味の拠点をAWS CloudFrontへ移す話](https://koudenpa.hatenablog.com/entry/2026/04/10/094719)（2026-04-10）— Azure CDN (Classic) のマネージド証明書廃止と Front Door への強制有料移行を受け、AWS CloudFront への移行を決断した経緯と所感
- [phpMyAdminをAWSのLambda関数URLで動かす](https://koudenpa.hatenablog.com/entry/2026/04/06/105622)（2026-04-06）— Lambda Web Adapter を使って phpMyAdmin を AWS Lambda で動かす構成と、Lambda の制約への対処法を解説
- [Aurora for MySQL r6g to r8g のパフォーマンス変化例](https://koudenpa.hatenablog.com/entry/2026/04/04/202642)（2026-04-04）— Aurora MySQL のインスタンスタイプを r6g から r8g へ変更した際のパフォーマンス実測レポート
...
### GitHub Copilot Agent Lab 参加 — Blazor + .NET 10 ハンズオン（2026-04）
`dotnet-presentations/vscode-github-copilot-agent-lab` をベースにした GitHub Copilot Agent Mode のハンズオンワークショップに参加。Blazor WebAssembly + .NET 10 製のソーシャルビンゴゲーム「Soc Ops」を題材に、コンテキストエンジニアリング・カスタムエージェント・マルチエージェント TDD を体験。フォーク先 (`agent-lab-dotnet`) は GitHub Pages で公開済み。
...
| 時期 | マイルストーン |
|------|-------------|
| 2026-04 | Azure CDN → AWS CloudFront 個人インフラ移行（Azure のホビーユーザー切り捨てを受け） |
| 2026-04 | phpMyAdmin on AWS Lambda 動作実証（Lambda Web Adapter 活用） |
| 2026-04 | GitHub Copilot Agent Lab ハンズオン参加（Blazor + .NET 10）|
